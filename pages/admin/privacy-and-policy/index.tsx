...
export default function PrivacyAndPolicy({ Page }: { Page: Page }) {
  ...
  const handleSubmit = (
    ...
    pageService
      .updatePage(PageId.PrivacyAndPolicy, values, token)
      .then((res) => {
  ...
  return (
    <DshboardTemplate header={<Header />}>
      <h1 className="mb-8">{t("Privacy And Policy Page")}</h1>
...
export const getServerSideProps = withIronSessionSsr(async function ({ locale, req }: any) {
  const res = await pageService.getPage(PageId.PrivacyAndPolicy);
